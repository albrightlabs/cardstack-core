...
class Upload
{
    ...
    public function __construct(?string $uploadDir = null, ?string $boardsDir = null)
    {
        $this->uploadDir = $uploadDir ?? dirname(__DIR__) . '/public/uploads';
        $this->boardsDir = $boardsDir ?? Config::getBoardsPath();
    ...
    /**
     * Get all referenced upload URLs from all boards
     */
    public function getReferencedUploads(): array
    {
        $referenced = [];
        $files = glob($this->boardsDir . '/*.json');
        ...
        foreach ($files as $file) {
            ...
            $board = json_decode($content, true);
            if (!is_array($board)) {
                continue;
            }

            // Scan all columns and cards
            foreach ($board['columns'] ?? [] as $column) {
                foreach ($column['cards'] ?? [] as $card) {
                    // Check cover image
                    if (!empty($card['coverImage'])) {
                        $filename = basename($card['coverImage']);
    ...
    /**
     * Get orphaned uploads (files not referenced by any card)
     */
