    ...
    var SearchUI = {
            ...
            modal.innerHTML =
                ...
                '<div class="search-modal-content">' +
                    '<div class="search-input-wrapper">' +
                        ...
                        '</svg>' +
                        '<input type="text" class="search-input" placeholder="Search cards..." autocomplete="off" />' +
        ...
        bindEvents: function() {
            ...
            this.searchResults.addEventListener('click', function(e) {
                ...
                if (result) {
                    var cardId = result.dataset.cardId;
                    var boardId = result.dataset.boardId;
                    if (cardId && boardId) {
                        self.close();
                        self.navigateToCard(boardId, cardId);
                    }
                }
            });
        ...
        renderResults: function(query) {
            ...
            var html = this.results.map(function(result, index) {
                ...
                var path = result.boardName + ' / ' + result.columnName;

                return '<div class="search-result' + (index === 0 ? ' selected' : '') + '" data-card-id="' + self.escapeHtml(result.id) + '" data-board-id="' + self.escapeHtml(result.boardId) + '" data-index="' + index + '">' +
                    '<div class="search-result-title">' + highlighted + '</div>' +
                    (preview ? '<div class="search-result-preview">' + preview + '</div>' : '') +
                    '<div class="search-result-path">' + self.escapeHtml(path) + '</div>' +
                '</div>';
            }).join('');
        ...
        selectCurrent: function() {
            if (this.selectedIndex >= 0 && this.results[this.selectedIndex]) {
                ...
                this.close();
                this.navigateToCard(result.boardId, result.id);
            }
        },

        navigateToCard: function(boardId, cardId) {
            // Check if we're already on this board
            if (typeof BoardApp !== 'undefined' && BoardApp.state && BoardApp.state.board && BoardApp.state.board.id === boardId) {
                // Already on the board, just open the card modal
                BoardApp.openCardModal(cardId);
            } else {
                // Navigate to the board with card parameter
